...
-- ============================================================
-- INTAKE SIMPLIFIED: PROBLEM CATEGORY ONLY, EVERYTHING ELSE
-- COLLECTED NATURALLY BY THE VOLUNTEER DURING THE CHAT
-- ============================================================
-- The upfront form (name/gender/location/phone/ITS number/first message)
-- was creating friction right at the point a woman is reaching out. She
-- now only picks a problem category before landing straight in the live
-- chat — no first message captured at creation either; her opening line
-- becomes an ordinary chat message via the existing send_message_as_seeker,
-- same as every message after it.
--
-- Gender is dropped from intake entirely rather than made volunteer-filled
-- like the rest — the round-robin assignment below was already gender-blind
-- (it never filtered by gender), so this was purely a display field with no
-- effect on routing.
--
-- name/location/phone/ITS number/problem_category all stay as columns —
-- they're just null until a volunteer fills them in via the dashboard as
-- she learns them (existing "volunteers update cases" RLS policy already
-- permits this, no new policy needed).
--
-- Defensively drops every historical signature this function has had, so
-- this runs cleanly regardless of which version is currently live.
drop function if exists start_conversation(text);
drop function if exists start_conversation(text, text, text, text, text);
drop function if exists start_conversation(text, text, text, text, text, text);
drop function if exists start_conversation(text, text, text, text, text, text, text);
...
begin
  ...
  -- note is not-null, and there's no first message to put there anymore —
  -- the problem category is a reasonable placeholder until a volunteer
  -- adds real context via case_updates.
  insert into cases (
...
-- ============================================================
-- get_conversation: fix for the simplified intake above
-- ============================================================
-- A freshly-started conversation now has zero messages (no more
-- first-message-at-creation), but this function used a plain `join` against
-- case_messages — meaning a brand new conversation with no messages yet
-- returned NO ROWS AT ALL, which the chat page reads as "code not found"
-- right after someone just created that exact code. Switched to a left
-- join so the case's own info (status/severity/who's assigned) always
-- comes back, with message fields simply null until a first message exists.
--
-- Also added two columns the frontend already expects but never actually
-- got: `assigned_role` (chat.html has always read this to show "Speaking
-- with a volunteer" vs "a counsellor", silently falling back to "Waiting
-- for a volunteer" every time since this column never existed here) and
-- `audio_path` (needed for voice-message playback — may already exist on
-- whatever version is actually live, but wasn't reflected in this file).
